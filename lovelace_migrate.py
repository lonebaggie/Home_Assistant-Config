...
import argparse import logging import os import shutil import sys from collections import OrderedDict import 
homeassistant.core from homeassistant.config import find_config_file, get_default_config_dir, 
load_yaml_config_file from homeassistant.util import yaml _LOGGER = logging.getLogger(__name__) GROUP_CONFIG = 
None def convert_all_group(obj_id):
    ...
    return lovelace def convert_card(entity_id):
    ...
    _LOGGER.error("Cannot determine card type for entity id '{}'. Maybe it is unsupported?"
                  "".format(entity_id))
    return None def convert_group(config, name):
    ...
    return lovelace, extra_cards def convert_view(config, name):
    lovelace = OrderedDict()
    if 'name' in config:
