...
unsetschedule= `curl  -X PUT -d "[{\"enabled\":false}]" -H "Content-Type: application/json" -H "Authorization: Bearer $token" https://www.services.renault-ze.com/api/vehicle/$vin/charge/scheduler/onboard`
echo $unsetschedule